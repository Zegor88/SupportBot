...
The file ships with a minimal example set: one behavioral rule (greeting), one static reply rule (farewell), and four test rules that trigger each action type (`drop`, `forward`, direct reply, prompt handoff), so every behaviour can be verified end to end.
